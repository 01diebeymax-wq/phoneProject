...
export const setupSocketIO = (server) => {
  ...
  const setupMeetingRoutes = (app) => {
    ...
    // Diagnostic endpoint for connection debugging
    app.get('/api/meeting/:meetingId/diagnostics', authenticateUser, (req, res) => {
      ...
      const diagnostics = {
        ...
        unhealthyConnections: meeting.getUnhealthyConnections(),
...
socket.on('join-meeting', async (data) => {
  ...
  console.log(`Participant ${participantName} joined meeting ${meetingId} ("${meeting.getMeetingName()}")`);
    ...
    socket.on('participant-ready', () => {
      ...
      console.log(`Participant ${socket.id} is ready for WebRTC connections. Ready participants: ${readyParticipants.length}`);

      // Notify all participants in the meeting that this user is ready
      socket.to(participantInfo.meetingId).emit('peer-ready', {
        socketId: socket.id,
      ...
      readyParticipants.forEach(participant => {
        if (participant.socketId !== socket.id) {
          ...
          const existingParticipantSocket = io.sockets.sockets.get(participant.socketId);

          if (!existingParticipantSocket) {
            console.warn(`Cannot initiate connection: participant ${participant.socketId} socket not found`);
            return;
          }

          // Existing participant should create offer to the new participant
          existingParticipantSocket.emit('initiate-connection', {
            ...
            targetParticipant: {
              name: meeting.participants.get(socket.id)?.name,
              socketId: socket.id
            }
          });

          // New participant should wait for offer
          ...
          console.log(`Initiating connection: ${participant.socketId} (offerer) -> ${socket.id} (answerer)`);
        }
      });
    });
    ...
    // Enhanced offer handling with better logging and direct socket delivery
    socket.on('offer', (data) => {
      ...
      if (!targetSocket) {
        console.error(`Target socket ${target} not found for offer from ${socket.id}`);
        socket.emit('signaling-error', {
      ...
      console.log(`Offer from ${socket.id} to ${target} (${connectionId}) - Type: ${offer?.type}, SDP length: ${offer?.sdp?.length || 0}`);

      // Use direct socket emission for reliability
    ...
    // Enhanced answer handling with direct socket delivery
    socket.on('answer', (data) => {
      ...
      const targetSocket = io.sockets.sockets.get(target);
      if (!targetSocket) {
        console.error(`Target socket ${target} not found for answer from ${socket.id}`);
        socket.emit('signaling-error', {
          message: 'Target participant not found',
          targetSocketId: target
        });
      ...
      console.log(`Answer from ${socket.id} to ${target} (${connectionId}) - Type: ${answer?.type}, SDP length: ${answer?.sdp?.length || 0}`);
    ...
    // Enhanced ICE candidate handling with direct socket delivery
    socket.on('ice-candidate', (data) => {
      const validation = validateData(data, ['target']);
      if (!validation.isValid) {
      ...
      if (!candidate || (candidate.candidate === undefined && candidate.candidate !== null)) {
        console.warn(`Invalid ICE candidate structure from ${socket.id}:`, candidate);
        return;
      }
      ...
      const targetSocket = io.sockets.sockets.get(target);
      if (!targetSocket) {
        console.warn(`Target socket ${target} not found for ICE candidate from ${socket.id}`);
        return;
      }
      ...
      if (candidate.candidate) {
        ...
        console.log(`ICE candidate from ${socket.id} to ${target} - Type: ${candidateType} (${connectionId})`);
      } else {
        console.log(`ICE gathering complete from ${socket.id} to ${target} (${connectionId})`);
      }
    ...
    // Renegotiation support for adding/removing tracks
    socket.on('renegotiation-needed', (data) => {
      const validation = validateData(data, ['targetSocketId']);
      if (!validation.isValid) {
        console.error(`Invalid renegotiation-needed data from ${socket.id}:`, validation.error);
        return;
      }
      ...
      if (!targetSocket) {
        console.warn(`Target socket ${targetSocketId} not found for renegotiation from ${socket.id}`);
        return;
      }

      console.log(`Renegotiation needed from ${socket.id} to ${targetSocketId}, reason: ${reason}`);

      targetSocket.emit('renegotiation-request', {
        fromSocketId: socket.id,
        reason: reason,
        connectionId: `renegotiate-${socket.id}-${targetSocketId}-${Date.now()}`
      });
    });

    // Track changes notification (when participant adds/removes audio or video)
    socket.on('track-changed', (data) => {
      const validation = validateData(data, ['trackType', 'enabled']);
      if (!validation.isValid) {
        return;
      }

      const { trackType, enabled } = data;
      const participantInfo = participants.get(socket.id);
      if (!participantInfo) return;
      ...
      console.log(`Track changed for ${socket.id}: ${trackType} ${enabled ? 'enabled' : 'disabled'}`);

      // Notify all other participants about track change
      socket.to(participantInfo.meetingId).emit('peer-track-changed', {
        socketId: socket.id,
        trackType: trackType,
        enabled: enabled
      });
    ...
    socket.on('disconnect', (reason) => {
      ...
      if (participantInfo) {
        ...
        if (meeting) {
          ...
          // Get list of all connected participants before removing
          ...
          if (participantInfo.isHost) {
            ...
            // Close all peer connections by notifying participants
            ...
            // Notify each peer to close their connection to this participant
            connectedParticipants.forEach(participantId => {
              const participantSocket = io.sockets.sockets.get(participantId);
              if (participantSocket) {
                participantSocket.emit('peer-disconnected', {
    ...
    // Connection verification - confirm both peers are ready
    socket.on('verify-peer-connection', (data) => {
      const validation = validateData(data, ['targetSocketId']);
      if (!validation.isValid) {
        return;
      }
      ...
      if (targetSocket) {
        targetSocket.emit('peer-connection-verify', {
          fromSocketId: socket.id
        });

        socket.emit('peer-verified', {
          targetSocketId: targetSocketId,
          verified: true
        });
      } else {
    ...
    socket.on('debug-signaling', (data) => {
      const { event, details } = data;
      console.log(`[DEBUG-SIGNALING] ${socket.id}: ${event}`, details);
    });

    // Client-side error reporting
    socket.on('client-error', (data) => {
      const { error, context, timestamp } = data;
      console.error(`[CLIENT-ERROR] ${socket.id}:`, error, 'Context:', context, 'Time:', timestamp);
    });

    // Media stream status update
    socket.on('media-stream-status', (data) => {
      const validation = validateData(data, ['hasAudio', 'hasVideo']);
      if (!validation.isValid) {
        return;
      }

      const { hasAudio, hasVideo, audioTracks, videoTracks } = data;
      const participantInfo = participants.get(socket.id);
      ...
      if (participantInfo) {
        ...
        if (meeting) {
          const participant = meeting.participants.get(socket.id);
          if (participant) {
            console.log(`Media stream status for ${socket.id}: Audio=${hasAudio}(${audioTracks}), Video=${hasVideo}(${videoTracks})`);

            // Broadcast media status to other participants
            socket.to(participantInfo.meetingId).emit('peer-media-status', {
              socketId: socket.id,
              hasAudio: hasAudio,
              hasVideo: hasVideo,
              participantName: participant.name
    ...
    socket.on('ping-peer', (data) => {
      const validation = validateData(data, ['targetSocketId']);
      if (!validation.isValid) {
        return;
      }

      const { targetSocketId, timestamp } = data;
      const targetSocket = io.sockets.sockets.get(targetSocketId);

      if (targetSocket) {
        targetSocket.emit('pong-peer', {
          fromSocketId: socket.id,
          timestamp: timestamp,
